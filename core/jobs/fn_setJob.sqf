/* RHD - LifeCore | Job / Duty System | Server */
params ["_unit", "_job"];

if (!isServer || {isNull _unit} || {!isPlayer _unit}) exitWith {};
if (owner _unit != remoteExecutedOwner) exitWith {};
if !(missionNamespace getVariable ["RHD_LIFE_ENABLE_JOBS", false]) exitWith {};
...
if (_def isEqualTo []) exitWith {
    ["Invalid job.", "error"] remoteExecCall ["RHD_fnc_notify", owner _unit];
};
...
[format ["Job changed to %1. You are now on duty.", _def select 0], "success"] remoteExecCall ["RHD_fnc_notify", owner _unit];
...
[_unit, _token] spawn {
    ...
    while {
        ...
        {!isNull _unit} &&
        {alive _unit} &&
        {missionNamespace getVariable ["RHD_LIFE_ENABLE_JOBS", false]} &&
        {_unit getVariable ["RHD_WORKING", false]} &&
        {_unit getVariable ["RHD_PAY_TOKEN", -1] isEqualTo _token}
    } do {
        sleep 60;
