...
if (_fuelNow >= 0.99) exitWith {["Your tank is already full.", "info"] remoteExecCall ["RHD_fnc_notify", owner _unit];};
private _unitsNeeded = ceil ((1 - _fuelNow) * 100);
private _cost = (_unitsNeeded * 5) min 750;
private _cash = _unit getVariable ["RHD_CASH", 0];
if (_cash < _cost) exitWith {[format ["Refuel costs $%1. You need more cash.", _cost], "error"] remoteExecCall ["RHD_fnc_notify", owner _unit];};

_unit setVariable ["RHD_CASH", _cash - _cost, true];
_vehicle setFuel 1;
[format ["Vehicle refueled for $%1.", _cost], "success"] remoteExecCall ["RHD_fnc_notify", owner _unit];
